...
async function runKCR() {
    ...
    try {
        ...
        // Exit the shell so onFinish fires and the action completes.
        // Without this, bash stays alive after the last command and
        // Komodo keeps the action in "ongoing" state indefinitely.
        {
            ...
            await komodo.execute_server_terminal(
                ...
                {
                    onLine: () => {},
                    onFinish: () => { shellExited = true; },
                }
            );
            const exitDeadline = Date.now() + 5000;
            while (!shellExited && Date.now() < exitDeadline) {
                await new Promise(r => setTimeout(r, 100));
            }
        }
        ...
        // The bash shell opened by `init` stays alive after commands finish,
        ...
        // the SDK promise would hang indefinitely, so we cap the wait at 2s.
        try {
            await Promise.race([
