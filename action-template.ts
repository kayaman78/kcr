...
async function runKCR() {
    ...
    try {
        ...
        // Always delete the terminal — even if a command failed or timed out.
        // No `init` here: the terminal already exists from the loop above
        // (or never was, if validation failed early — then DeleteTerminal is a no-op).
        try {
